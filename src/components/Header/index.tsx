...
const Header = () => {
  ...
  return (
  <header className={`${styles.glassEffect} w-full h-16 bg-gray-400 bg-opacity-40 flex items-center justify-between fixed z-10`}>
    <h1 className="text-independence md:text-4xl text-3xl ml-4 font-pixellari">Ribamar Filho</h1>
    {
      openMenu ? 
    ...
    <ul className={`text-rhythm md:text-3xl text-2xl font-pixellari md:flex md:static ${openMenu ? 'block absolute' : 'hidden'} md:top-0 top-16 gap-8 mr-4 text-center md:w-auto w-full md:bg-transparent md:backdrop-blur-0 md:shadow-none ${openMenu ? styles.glassEffect : ''}`}>
      <li className='md:py-0 py-2 hover:text-white md:hover:bg-transparent hover:bg-gray-400 duration-300 transition-colors cursor-pointer'>Start</li>
