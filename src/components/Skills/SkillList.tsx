...
const SkillList = () => {
  ...
  return <div className="flex items-center w-3/4 text-xl">
      ...
      <ul className={`flex flex-col lg:w-1/2 w-full border rounded-lg p-4 h-[75vh] overflow-y-auto bg-paper text-independence`}>
        { SKILL_LIST.map((skill, index) => (<div  key={index}>
          <li className="flex flex-row flex-none items-center py-2 cursor-pointer relative group overflow-hidden" onClick={() => handleReveal(index)} onMouseEnter={() => setContent(skill)} onMouseLeave={() => setContent(null)} >
            ...
            <h2>{skill.rating}</h2>
            <svg className={`w-7 h-7 ${revealedIndex === index ? 'rotate-90': ''}`} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd" /></svg>
          ...
          <p className={`text-justify ${revealedIndex === index ? 'block': 'hidden'}`}>{skill.description}</p>
          <div className="border-b border-b-independence"></div>
          </div>)) }
      </ul>
      <div className="w-1/2 h-[50vh] bg-paper text-independence rounded-lg lg:block hidden">
        { content ? 
